...
class TestMSTeamsBackendErrorHandling(DjangoTestCase):
    def setUp(self):
        self.project = Project.objects.create(name="Test project")
        self.config = MessagingServiceConfig.objects.create(
            project=self.project,
            display_name="Test MSTeams",
            kind="msteams",
            config=json.dumps({"webhook_url": "https://msteams.com/api/webhooks/test"}),
        )

    @patch('alerts.service_backends.base.BaseWebhookBackend.safe_post')
    def test_msteams_test_message_success_clears_failure_status(self, mock_post):
        # Set up existing failure status
        self.config.last_failure_timestamp = timezone.now()
        self.config.last_failure_status_code = 500
        self.config.last_failure_response_text = "Server Error"
        ...
        msteams_backend_send_test_message(
            "https://msteams.com/api/webhooks/test",
            "Test project",
            "Test MSTeams",
            self.config.id
        )
    ...
    @patch('alerts.service_backends.base.BaseWebhookBackend.safe_post')
    def test_msteams_test_message_non_json_error_stores_failure(self, mock_post):
        # Mock HTTP error response with non-JSON text
        mock_response = Mock()
        mock_response.status_code = 500
        mock_response.text = 'Internal Server Error'
    ...
    @patch('alerts.service_backends.base.BaseWebhookBackend.safe_post')
    def test_msteams_test_message_connection_error_stores_failure(self, mock_post):
        # Mock connection error
        mock_post.side_effect = requests.ConnectionError("Connection failed")
    ...
    @patch('alerts.service_backends.base.BaseWebhookBackend.safe_post')
    def test_msteams_alert_message_success_clears_failure_status(self, mock_post):
        # Set up existing failure status
        self.config.last_failure_timestamp = timezone.now()
        self.config.last_failure_status_code = 500
        self.config.save()
...
class TestWebhookConfigForms(DjangoTestCase):
    ...
    def test_msteams_form_rejects_blocked_target(self):
        form = MSTeamsConfigForm(data={"webhook_url": "http://10.0.0.42/hooks/test"})
